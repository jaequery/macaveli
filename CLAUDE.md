...
### Never Sleep + the SleepGuard root daemon

`DisplaySleepManager` (`src/Manager/DisplaySleepManager.swift`) owns the "Never
Sleep" control — three rungs: `.off`, `.whileLidOpen` (IOKit display-sleep
assertion, no root, released on quit), and `.evenLidClosed` (`pmset disablesleep
1`, needs root via an `osascript … with administrator privileges` prompt; the
only lever that survives a lid-close, and it can't be scoped to AC-only).

Because `.evenLidClosed` would otherwise strand the Mac awake on battery (lid
shut, in a bag, no screen to warn on), it's bounded from both ends:

- **Enable-gate:** `setMode` refuses to *enter* `.evenLidClosed` unless on AC
  (`onACPower()`, a synchronous `IOPSGetProvidingPowerSourceType` read).
- **Auto-revert:** a tiny root LaunchDaemon, **SleepGuard**, is registered via
  `SMAppService.daemon` while `.evenLidClosed` is active and unregistered when
  leaving it (`setSleepGuard`). Its *entire* job: on a power-source change, if on
  battery **and** `disablesleep == 1`, run `pmset -a disablesleep 0`. So an
  unplug auto-clears the dangerous state with no user present.

**SleepGuard security posture (by construction):** no IPC (takes no commands —
it can only ever *clear* the setting, never enable, so there's no channel to
drive a root process); no interpolated input (the one command is the fixed
literal `pmset -a disablesleep 0`); event-driven via
`IOPSNotificationCreateRunLoopSource` (blocks in `CFRunLoopRun`, ~0 CPU/battery
idle, never polls); and it ships **inside** the signed/notarized app bundle on a
root-owned, Gatekeeper-validated path (no user-writable launch target). Source:
`Helper/SleepGuard/main.swift`; launchd job: `Helper/com.jaequery.Macaveli.SleepGuard.plist`.
It logs to `/Library/Logs/Macaveli-SleepGuard.log`. First registration sends the
user to System Settings → Login Items to approve the background item.

**Build/packaging:** the helper is NOT an Xcode target — it's compiled + embedded
by `scripts/embed-sleepguard.sh`, called post-build from both `make build`
(Debug, `Apple Development`, non-hardened) and `scripts/build-dmg.sh` (Release,
Developer ID, hardened runtime + secure timestamp, so notarization accepts it).
The script builds a universal binary, drops it at `Contents/MacOS/MacaveliSleepGuard`
+ `Contents/Library/LaunchDaemons/`, signs it, then re-seals the app shell. It
derives the exact signing identity from the app's own signature to dodge the
"ambiguous Apple Development cert" trap. `make build` strips any prior embed
before `xcodebuild` runs so the incremental code-sign step never chokes on the
post-build helper.
