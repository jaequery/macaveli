...
class WindowManager {
    ...
    /// One-time cleanup: explicitly clears AXEnhancedUserInterface and
    /// AXManualAccessibility on every running app. We had a regression where
    /// earlier builds wrote these flags = true while trying to support
    /// Chromium/Electron, and the flags persist for the lifetime of the target
    /// process — leaving AppKit apps in "animate every kAXPosition/kAXSize
    /// write" mode, which manifests as slow drag and incremental maximize.
    /// Running this on launch undoes that damage without requiring a reboot.
    static func cleanupResidualAccessibilityFlags() {
