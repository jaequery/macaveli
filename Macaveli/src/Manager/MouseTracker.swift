...
class MouseTracker {
    ...
    /// Hard-stop any in-flight tracking regardless of which action started it.
    /// One-shot hotkeys call this so their position/size writes don't get
    /// immediately overwritten by a tracker that was started on the same
    /// modifier-down event (e.g. ⌃⌘ for move + ⌃⌘↑ for maximize).
    func forceStopTracking() {
    ...
    private func determineQuadrant(mouseLocation: NSPoint, windowSize: CGSize, windowLocation: NSPoint) -> Quadrant {
        ...
        let centreSize = 0.25 // https://github.com/jaequery/Macaveli/pull/54#discussion_r1635854368
